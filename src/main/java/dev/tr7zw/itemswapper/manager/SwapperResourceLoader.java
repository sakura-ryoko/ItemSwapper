...
public class SwapperResourceLoader implements net.fabricmc.fabric.api.resource.SimpleSynchronousResourceReloadListener,
    ...
    public void onResourceManagerReload(ResourceManager resourceManager) {
        ...
        resourceManager.listResources("itemgroups", id -> id.getPath().endsWith(".json")).forEach((id, resourceRef) -> {
            try {
                ...
                Entry<Identifier, JsonElement> entry = new AbstractMap.SimpleEntry<>(getResourceLocation(ids), data);
                processEntry(entry);
            } catch (Exception e) {
    ...
    private void processEntry(Entry<Identifier, JsonElement> entry) {
        try {
    ...
    private void processV2(Identifier jsonLocation, JsonElement json) {
    ...
    private void processList(Identifier jsonLocation, JsonObject json) {
        ...
        if (json.has("link") && json.get("link").isJsonPrimitive()) {
            try {
                group.withLink(getResourceLocation(json.getAsJsonPrimitive("link").getAsString()));
            } catch (Exception ex) {
    ...
    private void processPalette(Identifier jsonLocation, JsonObject json) {
        ...
        if (json.has("fallbackLink") && json.get("fallbackLink").isJsonPrimitive()) {
            try {
                group.withFallbackLink(getResourceLocation(json.getAsJsonPrimitive("fallbackLink").getAsString()));
            } catch (Exception ex) {
        ...
        if (json.has("forceLink") && json.get("forceLink").isJsonPrimitive()) {
            try {
                group.withForcedLink(getResourceLocation(json.getAsJsonPrimitive("forceLink").getAsString()));
            } catch (Exception ex) {
    ...
    private void processListModification(Identifier jsonLocation, JsonObject json) {
        ItemListModifier.Builder changes = ItemListModifier.builder();
        if (json.has("target") && json.get("target").isJsonPrimitive()) {
            try {
                changes.withTarget(getResourceLocation(json.getAsJsonPrimitive("target").getAsString()));
            } catch (Exception ex) {
    ...
    private void processPaletteModification(Identifier jsonLocation, JsonObject json) {
    ...
    private List<Shortcut> processShortcuts(Identifier jsonLocation, JsonElement object) {
        if (object == null) {
        ...
        object.getAsJsonArray().forEach(el -> {
            ...
            if (entry.has("type") && entry.get("type").isJsonPrimitive()
                ...
                try {
                    shortcuts
                            .add(new LinkShortcut(getResourceLocation(entry.getAsJsonPrimitive("target").getAsString()),
    ...
    private ItemEntry[] processItems(Identifier jsonLocation, JsonElement object) {
        if (object == null) {
            return null;
        }
        if (!object.isJsonArray()) {
    ...
    private void processCombined(Identifier jsonLocation, JsonElement json) {
        if (json == null) {
            return;
        }
        if (!json.isJsonArray()) {
    ...
    private Item[] getItemArray(Identifier jsonLocation, JsonElement json, boolean pallet) {
        if (json == null) {
            return null;
        }
        if (!json.isJsonArray()) {
    ...
    public static class ResourceLoaderInit {
        ...
        public static void init() {
            ...
            net.fabricmc.fabric.api.resource.v1.ResourceLoader.get(PackType.CLIENT_RESOURCES).registerReloadListener(
